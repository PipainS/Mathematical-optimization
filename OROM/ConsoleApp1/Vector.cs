...
namespace OROM
{
    public class Vector
    {
        ...
        public bool SetElement(double el)
        {
            int index = Size - 1;
            if (index < 0 || index >= size) return false;
            data[index] = el;
        ...
        {
            get { if (index < 0 || index >= size) return default; else return data[index]; }
            set { if (index >= 0 && index < size) data[index] = value; }
        }
        ...
        public Vector SubVector(int indMin, int indMax)
        {
            if (indMin >= 0 && indMax < size && indMin <= indMax)
            {
                int n = indMax - indMin + 1;
                Vector rez = new Vector(n);
                for (int i = 0; i < n; i++) rez.data[i] = data[i + indMin];
                return rez;
            }
            else return null;
        }
        public Vector ReplaceSubVector(Vector r, int indBegin)
        {
            if (indBegin >= 0 && (indBegin + r.GetSize() <= size))
            {
                Vector rez = this.Copy();
                for (int i = 0; i < r.GetSize(); i++) rez[indBegin + i] = r[i];
                return rez;
            }
            return null;
        }
        ...
        public Vector Substract(Vector c)
        {
            Vector rez = new Vector(size);
            for (int i = 0; i < size; i++) rez.data[i] = data[i] - c.data[i];
            return rez;
